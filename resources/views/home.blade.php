        ...
        <div class="col-lg-3 col-md-6 mb-4">
            <div class="card text-center h-100">
                <div class="card-body d-flex flex-column">
                    ...
                    <h3 class="fs-2 mb-2">{{ App\Models\Thread::where('is_approved', true)->count() }}</h3>
            ...
            <div class="card h-100">
                ...
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        @forelse(App\Models\Thread::with('user', 'category')->where('is_approved', true)->latest()->take(5)->get() as $thread)
                            <a href="{{ route('threads.show', $thread->id) }}" class="list-group-item list-group-item-action">
                                <div class="d-flex w-100 justify-content-between align-items-center">
                                    ...
                                    <div class="text-end">
                                        <small class="text-muted d-block"><i class="far fa-clock me-1"></i>{{ $thread->created_at->diffForHumans() }}</small>
                                        @if($thread->views_count > 0)
                                            <small class="text-muted"><i class="fas fa-eye me-1"></i>{{ $thread->views_count }} views</small>
                                        @endif
                                    </div>
                                </div>
                            </a>
                        @empty
                            <div class="text-center py-5">
                                ...
                                <h5 class="text-muted mb-2">Belum ada thread yang tersedia</h5>
                                <p class="text-muted mb-3">Jadilah yang pertama untuk memulai diskusi!</p>
                    ...
                    <div class="row text-center">
                        <div class="col-4">
                            <div class="border-end">
                                <div class="fw-bold text-primary">{{ Auth::user()->threads()->count() }}</div>
                                <small class="text-muted">Thread</small>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="border-end">
                                <div class="fw-bold text-success">{{ Auth::user()->comments()->count() }}</div>
                                <small class="text-muted">Komentar</small>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="fw-bold text-info">{{ Auth::user()->created_at->diffForHumans() }}</div>
            ...
            <div class="card">
                ...
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        ...
                        <a href="{{ route('threads.index') }}" class="list-group-item list-group-item-action py-3">
                            <div class="d-flex align-items-center">
                                <div class="rounded-circle bg-info d-flex align-items-center justify-content-center me-3"
                                    style="width: 40px; height: 40px; background: linear-gradient(to right, #4facfe, #00f2fe) !important;">
                                    <i class="fas fa-search text-white"></i>
                                </div>
                                <div>
                                    <h6 class="mb-0">Jelajahi Thread</h6>
                                    <small class="text-muted">Temukan diskusi menarik</small>
                                </div>
                            </div>
                        </a>
                    </div>
